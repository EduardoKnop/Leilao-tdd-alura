...
class LeilaoTest {
    ...
    fun newLance_LancesCheaperOrEqualsThanMaiorLance_ShouldNotBeAccepted() {
        ...
        var e: Exception
        e = assertThrows(RuntimeException::class.java) { bid1.newLance(Lance(user1, 400.0)) }
        assertEquals("Lance's Valor Cheaper than MaiorLance", e.message)
        e = assertThrows(RuntimeException::class.java) { bid1.newLance(Lance(user2, 450.0)) }
        assertEquals("Lance's Valor Cheaper than MaiorLance", e.message)
        e = assertThrows(RuntimeException::class.java) { bid1.newLance(Lance(user1, 100.0)) }
        assertEquals("Lance's Valor Cheaper than MaiorLance", e.message)
        e = assertThrows(RuntimeException::class.java) { bid1.newLance(Lance(user2, 500.0)) }
        assertEquals("Lance's Valor Cheaper than MaiorLance", e.message)
    ...
    fun newLance_SameUserCreateTwoOrMoreConsecutiveLances_ShouldNotBeAccepted() {
        ...
        var e: Exception
        e = assertThrows(RuntimeException::class.java) { bid1.newLance(Lance(Usuario("user-1"), 600.0)) }
        assertEquals("Lance's User was the Same that the Previously One", e.message)
        e = assertThrows(RuntimeException::class.java) { bid1.newLance(Lance(Usuario("user-1"), 700.0)) }
        assertEquals("Lance's User was the Same that the Previously One", e.message)
        assertEquals(1, bid1.getLances().size)
    }
    ...
    fun newLance_SameUserCreateMoreThanFiveLances_ShouldNotBeAccepted() {
        ...
        val e = assertThrows(RuntimeException::class.java) { bid1.newLance(Lance(user1, 600.0)) }
